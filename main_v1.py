...
if openai_key:
    ...
    selected_model = st.session_state['model'] # Default model

    if 'temp' not in st.session_state:
        # select temperature on a scale of 0.0 to 1.0
        # st.sidebar.write("## Text Generation Temperature")
        st.session_state['temp'] = st.sidebar.slider(
            t2("Temperature"),
            min_value=0.0,
            max_value=1.0,
            value=0.5)
    temperature = st.session_state['temp'] # Default temperature choosen

    if 'cache' not in st.session_state:
        # set use_cache in sidebar
        st.session_state['cache'] = st.sidebar.checkbox(t2("Cache Enabled"), value=True)
    use_cache = st.session_state['cache']
    ...
    if upload_own_data:
        ...
        if uploaded_file is not None:
            ...
            dataset = {"label": file_name, "url": uploaded_file_path}
        ...
        st.session_state['dataset'] = datasets[[dataset["label"]
